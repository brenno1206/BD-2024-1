...
where nome_bnc like 'Banco Central';
...
select count(t.natureza) as credito
from transacoes as t
where t.natureza like 'CREDITO';
#13. Listar os nomes dos usuários que realizaram transações bancárias no último mês.
select u.nome_user, count(t.id_trans)
from usuarios as u
left join transacoes as t
on u.id_user = t.usuario
left join contas as c
...
left join saldos as s
on c.agencia = s.agencia and c.conta = s.conta and c.cod_banco = s.cod_banco
where s.data_saldo = LAST_DAY(CURDATE())
group by u.nome_user;
